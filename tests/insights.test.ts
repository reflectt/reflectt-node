...
describe('computeScore', () => {
  ...
  it('adds severity boost for high', () => {
...
describe('ingestReflection', () => {
  it('creates an emerging insight from first reflection', () => {
    const ref = makeReflection()
    const insight = ingestReflection(ref)

    expect(insight.id).toMatch(/^ins-/)
    expect(insight.status).toBe('emerging')
  ...
  it('clusters second reflection into same insight', () => {
    ...
    const ref2 = makeReflection({ author: 'link', pain: 'More truncation issues' })
  ...
  it('promotes when 2 independent authors contribute', () => {
    ...
    expect(insight.promoted_at).not.toBeNull()
...
describe('cooldown', () => {
  ...
  it('tickCooldowns archives old cooldown entries', () => {
    const ref = makeReflection({ severity: 'critical', evidence: ['proof.log'] })
    const insight = ingestReflection(ref)

    // Set to cooldown with old timestamps
    const db = getDb()
  ...
  it('reopens cooldown insight when new reflection arrives', () => {
    ...
    // Put it in cooldown (still within window)
    const db = getDb()
...
describe('listInsights', () => {
  ...
  it('filters by status', () => {
    const ref = makeReflection({ severity: 'critical', evidence: ['proof.log'] })
    ingestReflection(ref) // promoted

    const { insights } = listInsights({ status: 'promoted' })
    expect(insights).toHaveLength(1)

    const { insights: emerging } = listInsights({ status: 'emerging' })
...
describe('insightStats', () => {
  it('returns aggregate stats', () => {
    ...
    expect(stats.by_status.promoted).toBe(1) // critical override
    expect(stats.by_status.emerging).toBe(1)
...
describe('findByCluster', () => {
  it('finds existing non-archived insight', () => {
    const ref = makeReflection()
    const ins = ingestReflection(ref)
    const key = extractClusterKey(ref)

    const found = findByCluster(key)
    expect(found).not.toBeNull()
    expect(found!.id).toBe(ins.id)
  })

  it('returns null when no match', () => {
    const found = findByCluster({ workflow_stage: 'x', failure_family: 'y', impacted_unit: 'z' })
  ...
  it('ignores archived insights', () => {
    ...
    const db = getDb()
    db.prepare('UPDATE insights SET status = ? WHERE id = ?').run('archived', ins.id)

    const key = extractClusterKey(ref)
    expect(findByCluster(key)).toBeNull()
  })
})
