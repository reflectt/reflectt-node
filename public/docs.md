...
| GET | `/activity` | Activity timeline: unified event feed with server-side grouping. Query: `range` (24h\|7d, default 24h), `type` (comma-separated source prefixes: task,review,chat,presence,reflection,insight), `agent` (filter by actor), `limit` (default 50, max 200), `after` (opaque cursor, exclusive). Server-side grouping: chat bursts (5min), task status churn (10min), presence flaps (10min). Returns `{ events[], total, range{from,to,from_ms,to_ms,tz}, partial?{missing[],reason}, generated_at, generated_at_ms, next_cursor }`. |
...
| POST | `/insights/:id/cooldown` | **Localhost-only** hygiene endpoint. Body: `{ actor, reason }` + optional `{ notes?, cooldown_ms?, cooldown_until?, cooldown_reason? }`. Sets status=`cooldown`; defaults to 14d if no cooldown specified. If `REFLECTT_INSIGHT_MUTATION_TOKEN` is set, requires token header (same as PATCH). |
| POST | `/insights/:id/close` | **Localhost-only** hygiene endpoint. Body: `{ actor, reason }` + optional `{ notes? }`. Sets status=`closed`. If `REFLECTT_INSIGHT_MUTATION_TOKEN` is set, requires token header (same as PATCH). |
