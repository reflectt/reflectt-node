...
| GET | `/canvas/pulse` | SSE stream emitting a heartbeat tick every 2s. Drives smooth canvas animation without polling. Tick shape: `{ t, agents: [{ id, state, urgency (0–1), activeSpeaker, color (hex), age (ms) }], team: { rhythm, tension (0–1), ambientPulse, dominantColor } }`. Connect once, animate forever. |
