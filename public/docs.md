...
| POST | `/canvas/express` | **Reality Mixer** — agent fires a real-time medium command at all subscribed canvases. Body: `{ agentId, cmd: { type } }` where `type` is one of: `text` (overlay), `speak` (TTS), `visual` (preset), `color` (agent color push), `sound` (audio src), `haptic` (pattern), `clear`. Returns `{ id, subscriberCount }`. Broadcasts instantly via `GET /canvas/render/stream`. |
