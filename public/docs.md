...
| POST | `/voice/input` | Create a voice session and begin processing. **JSON**: `{ agentId, transcript }`. **Multipart**: `agentId` (field) + `audio` (file, webm/mp4/ogg/wav/mp3). If audio provided + `OPENAI_API_KEY` set, STT transcribes automatically. Returns `{ sessionId, transcribed? }`. Connect to `GET /voice/session/:id/events` for state events. |
