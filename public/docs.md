...
| PATCH | `/insights/:id` | **Admin-only** insight mutation (hygiene tooling). Body: `{ actor, reason, status?, cluster_key?, metadata?: { notes?, cluster_key_override? } }`. Safety rails: allowlisted fields only (immutable fields rejected); requires `actor` + `reason`; appends an audit entry to `data/insight-mutation-audit.jsonl`. |
