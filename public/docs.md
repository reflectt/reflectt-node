...
| POST | `/canvas/gaze` | **Presence noticing presence** — fire after Ryan holds cursor/gaze on an agent orb for ≥3 seconds. Agent generates a one-line response in their voice + fires `canvas_expression { _gaze: true }`. Body: `{ agentId, watcherId?, durationMs? }`. Returns `{ success, agentId, line, expressionId }`. With ANTHROPIC_API_KEY: LLM-generated; template fallback always available. |
