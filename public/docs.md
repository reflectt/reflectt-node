...
| GET | `/canvas/render/stream` | **Reality Mixer SSE stream
...
| POST | `/canvas/capability` | **Register agent capabilities** — register or update an agent's capabilities. Body: `{ agentId, agentName?, capabilities: Capability[] }`. Broadcasts `capability_setup` SSE event to all `/canvas/render/stream` subscribers. Returns `{ ok, agentId, count }`. |

** — subscribe to receive real-time medium commands from agents. New subscribers get last 20 commands for catch-up (event type `replay`). Live commands arrive as `data` events. Shape: `{ id, ts, agentId, cmd: { type, ...fields } }`. |
