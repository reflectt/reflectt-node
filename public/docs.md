...
| GET | `/doctor` | Structured host diagnosis with per-area recovery suggestions. Covers bootstrap stalls, crash loops, agent health, channel connectivity, error rates. Returns `{ healthy, diagnoses[{ area, status, message, recovery? }], next_action, stats, version }`. Use for operator triage when you need actionable per-area detail beyond `/host/status`. |
...
| GET | `/agents/:name/identity` | Host-native agent identity resolution. Resolves by name, alias, or display name without requiring OpenClaw gateway. Returns flat keys (`agentId`, `displayName`, `role`, `aliases`, `affinityTags`, `wipCap`, `source`) plus per-field DetailField envelope `fields.{name,displayName,role,description,aliases,affinityTags,wipCap,avatar,voice,color}` (each `{ value?, support, source }`) and a 16-char SHA-256 `revision` for optimistic concurrency. |
| PATCH | `/agents/:name/identity` | Per-field identity update — terminus of the cloud→openclaw-channel→reflectt-node write seam. Body: any subset of `{ displayName, role, description, aliases, affinityTags, wipCap, avatar, voice, color }`. Header `If-Match: <revision>` enables optimistic concurrency (412 with `currentRevision` on stale). Rename rejected — use `POST /agents/:name/identity/claim`. Returns the same envelope as GET with new revision. Emits `agent_identity_changed` SSE event. |
...
| POST | `/agents/:name/identity/claim` | **Identity handoff** — bootstrap agent claims real name + avatar + voice. Body: `{ claimedName, displayName?, voice?, avatar? }`. Atomically updates TEAM-ROLES.yaml, stores avatar/voice in agent_config, triggers gateway reidentify, emits `agent_identity_changed` SSE event. Use after human picks identity during first-boot setup. |
