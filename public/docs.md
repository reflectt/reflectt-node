...
| GET | `/canvas/flow-score` | Real-time team flow state (0–1). Composite of agent presence, task velocity, canvas expression activity, and time-of-day. Returns `{ score, label (flow|active|building|quiet), factors: { agents, velocity, expressions, timeOfDay }, meta, updatedAt }`. <50ms, safe to poll at 30s intervals. |
