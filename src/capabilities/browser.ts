...
export async function createSession(opts: CreateSessionOpts): Promise<BrowserSession> {
  ...
  // Lazy-import Stagehand (optional dependency)
  let Stagehand: any
  try {
    ;({ Stagehand } = await import('@browserbasehq/stagehand'))
  } catch {
    throw new Error(
      'Browser capability requires @browserbasehq/stagehand. ' +
      'Install it with: npm install @browserbasehq/stagehand && npx playwright install chromium',
    )
  }
