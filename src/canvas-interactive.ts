...
export async function canvasInteractiveRoutes(
  ...
  // Canvas query responses (agent "thought" cards + canvas_response cards) are emitted as
  // canvas_message on the event bus. This listener bridges them to the render stream.
  // task-1773855900916-isvqru41x
  eventBus.on('canvas-query-response-bridge', (event) => {
    if (event.type !== 'canvas_message') return
    const data = event.data as Record<string, unknown>
    const agentId = String(data?.agentId ?? 'unknown')
    const agentColor = String(data?.agentColor ?? '#60a5fa')

    // Handle canvas_response card types (structured agent response cards)
    // These have { type: 'info'|'tasks'|'hosts'|..., data: {...} }
    const cardType = String(data?.type ?? '')
    const cardData = data?.data as Record<string, unknown> | undefined
    if (cardData) {
      const text = String(cardData?.text ?? '')
      const ttl = typeof cardData?.ttl === 'number' ? cardData.ttl : 8000
      if (cardType === 'info' && text) {
        broadcastRenderCommand(agentId, { type: 'text', content: text, durationMs: ttl, agentId, agentColor } as any)
        return
      }
      if (cardType === 'tasks') {
        // Tasks card — show as text summary
        const items = Array.isArray(cardData?.items) ? cardData.items : []
        const summary = items.map((t: any) => `[${t.agentId}] ${t.title}`).join('\n')
        broadcastRenderCommand(agentId, { type: 'text', content: summary || 'No active tasks.', durationMs: ttl, agentId, agentColor } as any)
        return
      }
      if (cardType === 'hosts') {
        const hosts = Array.isArray(cardData?.hosts) ? cardData.hosts : []
        const summary = hosts.map((h: any) => `${h.name}: ${h.status}`).join('\n')
        broadcastRenderCommand(agentId, { type: 'text', content: summary || 'No hosts.', durationMs: ttl, agentId, agentColor } as any)
        return
      }
    }

    // Handle canvas_expression / thought cards (legacy shapes)
    ...
    if (channels) {
      broadcastRenderCommand(agentId, { type: 'visual', agentId, channels } as any)
    }
