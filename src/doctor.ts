...
export async function collectDoctorReport(input: {
  ...
  if (freshInstall) {
    ...
    hints.push('')
    hints.push('Required:')
    hints.push('  export ANTHROPIC_API_KEY=<your-key>   # or OPENAI_API_KEY=<your-key>')
...
export function formatDoctorHuman(report: DoctorReport): string {
  ...
  if (report.setupMode) {
    ...
    lines.push('   export ANTHROPIC_API_KEY=<your-key>   # Anthropic Claude')
    lines.push('   # or: export OPENAI_API_KEY=<your-key>    # OpenAI')
