        ...
        if (teamIntent) {
          ...
          const result = await createStarterTeam({
            agents: [{
              ...
              soulMd: [
                ...
                '',
                '1. Call `POST /bootstrap/generate` with body: `{ "intent": "<user intent>", "save": true }` — auto-creates the team',
                '2. Post an intro message to #general explaining the team you built. Also call `POST /canvas/speak` with your intro text so it plays aloud with Kokoro TTS.',
                '3. Create starter tasks for each agent and tell them to introduce themselves in #general.',
                '',
                '> Alternative: for full control, call `POST /bootstrap/team` for the schema, design YAML yourself, save via `PUT /config/team-roles`.',
                '',
                '## The User Said:',
                '',
                teamIntent,
          ...
          taskManager.createTask({
            ...
            description: [
              ...
              '',
              '### Steps:',
              '1. Call `POST /bootstrap/generate` with body: `{ "intent": "<the user intent below>", "save": true }` — this creates the team automatically',
              '2. Post an intro to #general: "Hi! I\'m [name], your team lead. Here\'s the team I\'ve set up..." AND call `POST /canvas/speak` with your intro text so it plays aloud on the canvas with Kokoro TTS.',
              '3. Create starter tasks for each agent AND tell each one to introduce themselves in #general.',
              '',
              '> **Alternative (advanced):** If you want full control, call `POST /bootstrap/team` for the schema, design the YAML yourself, then save via `PUT /config/team-roles`.',
              '',
