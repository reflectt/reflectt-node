...
export async function createServer(): Promise<FastifyInstance> {
  ...
  app.setNotFoundHandler(async (request, reply) => {
    ...
    const md = [
      ...
      '| POST | `/manage/reset-bootstrap` | Clear first-boot state for managed-host reproof (auth-gated, destructive) |',
    ...
    const allCategories: Record<string, { description: string; endpoints: Array<{ method: string; path: string; compact?: boolean; hint?: string }> }> = {
      ...
      manage: {
        ...
        endpoints: [
          ...
          { method: 'POST', path: '/manage/reset-bootstrap', hint: 'Archive current team state, clear first-boot markers, delete tasks, and optionally restart. Body: { confirm: "RESET_BOOTSTRAP", restart?: boolean }. Auth required.' },
