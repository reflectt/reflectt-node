...
<main><pre><code>${escapeHtml(result.file.content)}</code></pre></main>
  ...
  // Generates optimal HEARTBEAT.md content based on available endpoints.
  // New installs get the right config; existing agents can diff and self-update.
  app.get<{ Params: { agent: string } }>('/bootstrap/heartbeat/:agent', async (request) => {
    ...
    // Detect active lane from current doing tasks
    const doingTasks = taskManager.listTasks({ status: 'doing', assignee: agent })
    const activeLane = doingTasks.length > 0
      ? ((doingTasks[0].metadata as Record<string, unknown>)?.lane as string || 'general')
      : 'general'
...
## Priority Order
1. Check active work first (prevents stale context):
   - \`curl -s "http://127.0.0.1:4445/tasks/active?agent=${agent}&compact=true"\`
   - If no active tasks: \`curl -s "http://127.0.0.1:4445/tasks/next?agent=${agent}&compact=true"\`
2. If any task exists, **do real work first** (ship code/docs/artifacts).
3. Check inbox briefly: \`curl -s "http://127.0.0.1:4445/inbox/${agent}"\`
4. Respond to direct mentions.
5. **Never report task status from memory alone** — always query the API first.
...
## API Quick Reference
- Active task: \`GET /tasks/active?agent=${agent}&compact=true\`
- Next task: \`GET /tasks/next?agent=${agent}&compact=true\`
