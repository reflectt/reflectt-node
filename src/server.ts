...
import { getVoiceSession, subscribeVoiceSession } from './voice-sessions.js'
...
<main><pre><code>${escapeHtml(result.file.content)}</code></pre></main>
  ...
  // POST /voice/input — disabled until the voice loop is routed through real agents.
  // Body: { agentId: string, transcript?: string }
  app.post('/voice/input', async (_request, reply) => {
    reply.status(410)
    return { success: false, message: 'Voice conversation is disabled until routed through real agents.' }

  })

  // POST /voice/audio — disabled until the voice loop is routed through real agents.
  // Form fields: agentId (string), audio (file: wav/mp3/webm/ogg/m4a)
  app.post('/voice/audio', async (_request, reply) => {
    reply.status(410)
    return { success: false, message: 'Voice conversation is disabled until routed through real agents.' }
  })
