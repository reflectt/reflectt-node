...
1. Check status (single call — replaces 3 separate checks):
   - \`curl -s "http://127.0.0.1:4445/heartbeat/${agent}"\`
...
`
  ...
  // ── Capabilities: endpoint discovery for agents ────────────────────
  app.get('/capabilities', async () => {
    return {
      ...
      endpoints: {
        heartbeat: {
          poll: { method: 'GET', path: '/heartbeat/:agent', description: 'Single compact heartbeat payload (~200 tokens). Replaces 3 separate calls.' },
          bootstrap: { method: 'GET', path: '/bootstrap/heartbeat/:agent', description: 'Generate optimal HEARTBEAT.md. Re-fetch when version changes.' },
        },
        tasks: {
          list: { method: 'GET', path: '/tasks', compact: true },
          get: { method: 'GET', path: '/tasks/:id', compact: true },
          active: { method: 'GET', path: '/tasks/active', compact: true },
          next: { method: 'GET', path: '/tasks/next', compact: true },
        ...
        chat: {
          ...
          context: { method: 'GET', path: '/chat/context/:agent', description: 'Compact deduplicated chat. Always slim.' },
        ...
        insights: {
          ...
          summary: { method: 'GET', path: '/loop/summary', compact: true },
        },
        reflections: {
        ...
        system: {
          ...
          me: { method: 'GET', path: '/me/:agent', compact: true },
        },
      },
      recommendations: [
        'Use /heartbeat/:agent for heartbeat polls instead of 3 separate calls',
        'Add ?compact=true to GET requests to reduce tokens by 50-75%',
        'Use /chat/context/:agent instead of /chat/messages for context injection',
